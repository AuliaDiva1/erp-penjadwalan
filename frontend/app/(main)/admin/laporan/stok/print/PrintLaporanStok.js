...
export default function PrintLaporanStok({
  visible,
  onHide,
  setPdfUrl,
  setFileName,
  ...
  const generatePDF = (materials) => {
    ...
    doc.text("Laporan Monitoring Stok Bahan Baku", pw / 2, y + 10, { align: "center" });
    ...
    autoTable(doc, {
      ...
      didDrawPage({ pageNumber }) {
        ...
        doc.text(
          `Halaman ${pageNumber} dari ${count}  |  Jadwalin ERP`,
          pw - mR,
          ph - 6,
          { align: "right" }
        );
    ...
    doc.text("Administrator,", ttdX, ttdY + 10);
    ttdY += 28;
    doc.setFont("helvetica", "bold");
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const name = `Laporan_Stok_${dateTag}.pdf`;
