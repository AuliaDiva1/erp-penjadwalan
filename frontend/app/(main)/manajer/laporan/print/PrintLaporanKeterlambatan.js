...
const getSelisihMenit = (scheduled_end, actual_end) => {
  if (!scheduled_end || !actual_end) return "-";
  const diff = Math.round((new Date(actual_end) - new Date(scheduled_end)) / 60000);
  return diff > 0 ? `+${diff} mnt` : `${diff} mnt`;
};

export default function PrintLaporanKeterlambatan({
  ...
  const fetchData = async () => {
    ...
    const res = await fetch(`${BASE_URL}/jobs/report/keterlambatan?${params}`, {
  ...
  const generatePDF = (data, summary) => {
    ...
    doc.text(`Laporan Keterlambatan Job — Periode: ${getPeriodeLabel()} — Dicetak: ${todayStr}`, pw / 2, y + 10, { align: "center" });
    ...
    // RINGKASAN
    const urgentCount = data.filter(j => j.is_urgent).length;
    const summaryItems = [
      { label: "Total Terlambat", value: summary.total,  color: [239, 68, 68]  },
      { label: "Job Urgent",      value: urgentCount,     color: [249, 115, 22] },
      { label: "Non-Urgent",      value: summary.total - urgentCount, color: [107, 114, 128] },
    ];
    ...
    const rows = data.map((j, idx) => [
      ...
      getSelisihMenit(j.scheduled_end, j.actual_end),
    ]);

    autoTable(doc, {
      startY: y,
      head: [["No", "Job ID", "Operasi", "Mesin", "Material", "Status", "Urgent", "Jadwal Selesai", "Aktual Selesai", "Selisih"]],
      body: rows,
      margin: { left: mL, right: mR, bottom: mB + 10 },
      styles: { fontSize: 7.5, cellPadding: 2.5, lineColor: [230, 230, 230], lineWidth: 0.1 },
      ...
      didParseCell(data) {
        if (data.section === "body") {
          if (data.column.index === 6 && data.cell.raw === "Ya") data.cell.styles.textColor = [239, 68, 68];
          if (data.column.index === 9) {
            data.cell.styles.textColor = [239, 68, 68];
            data.cell.styles.fontStyle = "bold";
          }
          if (data.column.index === 0) data.cell.styles.fillColor = [254, 242, 242];
        }
      },
    ...
    doc.setFontSize(8.5); doc.setTextColor(120, 30, 30); doc.setFont("helvetica", "italic");
    doc.text("Catatan: Laporan ini memuat job dengan deadline_warning aktif (actual_end melebihi scheduled_end). Perlu ditindaklanjuti untuk evaluasi kinerja produksi.", mL, noteY, { maxWidth: pw - mL - mR });
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) { alert(`Tidak ada data keterlambatan untuk periode ${getPeriodeLabel()}.`); return; }
      const doc = generatePDF(data, summary);
      const name = `Laporan_Keterlambatan_${toMySQLDate(dateFrom) || "all"}_sd_${toMySQLDate(dateTo) || "now"}.pdf`;
  ...
  return (
    <Dialog
      ...
      header={<div className="flex align-items-center gap-2"><i className="pi pi-clock" style={{ color: "#ef4444" }} /><span>Cetak Laporan Keterlambatan</span></div>}
      ...
      footer={
        <div className="flex justify-content-end gap-2">
          ...
          <Button label="Generate PDF" icon="pi pi-print" onClick={handleGenerate} loading={loading} style={{ background: "#ef4444", borderColor: "#ef4444" }} />
        </div>
      }
    >
      <div className="grid p-fluid">
        <div className="col-12">
          <p className="text-color-secondary text-sm mt-0">Laporan khusus job yang melewati waktu jadwal (deadline_warning aktif), untuk evaluasi kinerja produksi.</p>
        </div>
        <div className="col-6">
          <div className="field">
