...
export default function PrintLaporanPengadaanGudang({
  visible,
  onHide,
  setPdfUrl,
  setFileName,
  ...
  const drawAccentTitle = (doc, text, x, y, color = [99, 102, 241], fontSize = 10) => {
  ...
  const generatePDF = (data, summary) => {
    ...
    doc.text(
      `Laporan Pengadaan Bahan Baku — Gudang - Periode: ${getPeriodeLabel()} - Dicetak: ${todayStr}`,
      pw / 2, y + 10,
      { align: "center" }
    );
    ...
    doc.text("Kepala Gudang,",             ttdX, ttdY + 5);
    ttdY += 22;
    doc.setFont("helvetica", "bold");
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const name = `Laporan_Pengadaan_Gudang_${getPeriodeLabel().replace(/ /g, "_")}.pdf`;
    ...
    <Dialog
      ...
      header={
        <div className="flex align-items-center gap-2">
          <i className="pi pi-shopping-cart" style={{ color: "#6366f1" }} />
