...
const LOGIN_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .lp-orb-2 {
    ...
    top: 10%; right: 12%;
    animation: orbFloatReverse 7s ease-in-out infinite; animation-delay: 1s;
  }
  .lp-orb-3 {
    width: 90px; height: 90px;
    background: radial-gradient(circle, rgba(255,255,255,0.06) 0%, transparent 70%);
    bottom: 18%; right: 22%;
    animation: orbFloat 8s ease-in-out infinite; animation-delay: 2s;
  }
  .lp-orb-4 {
    width: 55px; height: 55px;
    background: rgba(255,255,255,0.06);
    top: 52%; right: 8%;
    animation: orbFloatReverse 5s ease-in-out infinite; animation-delay: 0.5s;
  }
  .lp-brand {
  ...
  .lp-brand-name { font-size: 1.25rem; font-weight: 800; color: #fff; letter-spacing: -0.02em; line-height: 1.2; }
  .lp-brand-sub { font-size: 11px; color: rgba(199,210,254,0.7); font-weight: 500; margin-top: 1px; }
  .lp-left-body { position: relative; z-index: 1; animation: fadeUp 0.7s 0.35s ease both; }
  .lp-left-title {
    font-size: clamp(2rem, 3.2vw, 2.8rem);
    font-weight: 900; color: #fff;
    line-height: 1.12; letter-spacing: -0.04em; margin-bottom: 18px;
  }
  .lp-left-title em { font-style: normal; color: rgba(199,210,254,0.9); }
  .lp-left-desc { font-size: 15px; color: rgba(255,255,255,0.5); line-height: 1.85; font-weight: 400; max-width: 320px; }
  .lp-divider {
    width: 56px; height: 4px;
    background: linear-gradient(90deg, rgba(255,255,255,0.5), rgba(255,255,255,0.1));
    border-radius: 99px; margin: 22px 0; transform-origin: left;
    animation: lineGrow 0.6s 0.8s ease both;
  }
  .lp-chips { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 8px; }
  .lp-chip {
    background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.15);
    border-radius: 99px; padding: 7px 16px; font-size: 12.5px; font-weight: 600;
    color: rgba(255,255,255,0.8); animation: chipIn 0.5s ease both;
  ...
  .lp-cards { display: flex; gap: 14px; margin-top: 36px; position: relative; z-index: 1; }
  .lp-card {
    flex: 1; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.13);
    border-radius: 18px; padding: 18px 16px; backdrop-filter: blur(8px); text-align: center;
  }
  ...
  .lp-card-val { font-size: 1.8rem; font-weight: 900; color: #fff; letter-spacing: -0.04em; line-height: 1; }
  .lp-card-lbl { font-size: 10.5px; color: rgba(255,255,255,0.5); font-weight: 600; margin-top: 5px; text-transform: uppercase; letter-spacing: 0.06em; }
  .lp-card-icon { font-size: 18px; color: rgba(199,210,254,0.7); margin-bottom: 8px; }
  .lp-left-footer {
  ...
  .lp-left-footer-text { font-size: 12px; color: rgba(255,255,255,0.3); font-weight: 500; }
  .lp-left-footer-dot {
    width: 7px; height: 7px; border-radius: 50%; background: #4ade80;
    box-shadow: 0 0 8px rgba(74,222,128,0.6); animation: pulse 2s ease-in-out infinite;
  }
  .lp-left-footer-status { font-size: 11.5px; color: rgba(255,255,255,0.4); display: flex; align-items: center; gap: 6px; }
  ...
  .lp-form-box { width: 100%; max-width: 440px; position: relative; z-index: 1; }
  .lp-welcome-badge {
    display: inline-flex; align-items: center; gap: 7px;
    background: linear-gradient(135deg, #ede9fe, #ddd6fe);
    border: 1px solid #c4b5fd; border-radius: 99px; padding: 7px 16px;
    font-size: 12px; font-weight: 700; color: #6d28d9; margin-bottom: 22px;
    animation: fadeDown 0.5s 0.25s ease both; letter-spacing: 0.02em;
  }
  .lp-welcome-badge i { font-size: 11px; }
  .lp-form-title {
    font-size: 2rem; font-weight: 900; color: #0f172a; letter-spacing: -0.04em;
    margin-bottom: 8px; line-height: 1.15; animation: fadeDown 0.5s 0.35s ease both;
  }
  .lp-form-sub {
    font-size: 14.5px; color: #94a3b8; font-weight: 500;
    margin-bottom: 36px; line-height: 1.65; animation: fadeDown 0.5s 0.42s ease both;
  }
  ...
  .lp-label { display: flex; align-items: center; justify-content: space-between; margin-bottom: 9px; }
  .lp-label-text { font-size: 12.5px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.08em; }
  .lp-input-wrap { position: relative; }
  .lp-input {
    width: 100%; height: 56px; border-radius: 14px; border: 2px solid #e8ecf4; background: #f8faff;
    padding: 0 50px; font-family: 'Plus Jakarta Sans', sans-serif;
  ...
  .lp-input:focus {
    ...
    box-shadow: 0 0 0 4px rgba(124,58,237,0.1); transform: translateY(-1px);
    animation: inputFocusRipple 0.4s ease;
  }
  .lp-input:disabled { opacity: 0.55; cursor: not-allowed; transform: none; }
  .lp-input-icon {
    position: absolute; left: 17px; top: 50%; transform: translateY(-50%);
    color: #c0c9d8; pointer-events: none; font-size: 16px; transition: color 0.2s, transform 0.2s;
  }
  .lp-input-wrap:focus-within .lp-input-icon { color: #7c3aed; transform: translateY(-50%) scale(1.1); }
  .lp-eye {
    position: absolute; right: 16px; top: 50%; transform: translateY(-50%);
    background: none; border: none; cursor: pointer; color: #c0c9d8; padding: 4px;
  ...
  .lp-strength-bar { display: flex; gap: 4px; margin-top: 8px; }
  .lp-strength-seg { flex: 1; height: 3px; border-radius: 99px; background: #e8ecf4; transition: background 0.3s; }
  .lp-strength-seg.active { background: #7c3aed; }
  .lp-btn {
    width: 100%; height: 58px; border-radius: 14px; border: none; color: #fff;
    font-family: 'Plus Jakarta Sans', sans-serif; font-size: 15.5px; font-weight: 800; cursor: pointer;
    ...
    display: flex; align-items: center; justify-content: center;
    gap: 10px; margin-top: 10px; letter-spacing: -0.01em; position: relative; overflow: hidden;
  }
  .lp-btn::before {
    content: ''; position: absolute; inset: 0;
    background: linear-gradient(180deg, rgba(255,255,255,0.08) 0%, transparent 100%); border-radius: 14px;
  }
  ...
  .lp-btn:active:not(:disabled) { transform: translateY(0); box-shadow: 0 6px 18px rgba(91,33,182,0.3); }
  .lp-btn:disabled { background: #e2e8f0; animation: none; box-shadow: none; cursor: not-allowed; color: #94a3b8; }
  .lp-btn:disabled::before { display: none; }
  .lp-spinner {
    width: 20px; height: 20px; border: 2.5px solid rgba(255,255,255,0.3);
    border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite;
  }
  .lp-or { display: flex; align-items: center; gap: 12px; margin: 26px 0; animation: fadeIn 0.5s 0.7s ease both; }
  .lp-or-line { flex: 1; height: 1px; background: #f1f5f9; }
  .lp-or-text { font-size: 12px; color: #cbd5e1; font-weight: 600; }
  .lp-info-row { display: flex; gap: 10px; margin-top: 6px; animation: fadeUp 0.5s 0.72s ease both; }
  .lp-info-card {
    flex: 1; background: #f8faff; border: 1.5px solid #e8ecf4; border-radius: 12px;
    padding: 14px 12px; text-align: center; transition: border-color 0.2s, transform 0.2s;
  }
  .lp-info-card:hover { border-color: #c4b5fd; transform: translateY(-2px); }
  ...
  .lp-form-footer {
    text-align: center; margin-top: 28px; font-size: 13px; color: #94a3b8; font-weight: 500;
    animation: fadeUp 0.5s 0.78s ease both;
  }
  .lp-form-footer a { color: #7c3aed; font-weight: 700; text-decoration: none; transition: color 0.2s; }
  ...
  .lp-security {
    ...
    font-size: 11.5px; color: #cbd5e1; font-weight: 500; animation: fadeIn 0.5s 0.85s ease both;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post(
        'http://localhost:8100/api/auth/login', // ← hardcode sementara
        { email, password }
      );
      const data = res.data;
