...
def _compute_fitness(chromosome, jobs, machines, cache, mro=None):
    ...
    # Penalty untuk mesin yang masih busy (mro > 0) — dorong ke mesin yang free
    busy_penalty = 0.0
    if mro:
...
def _inisialisasi_populasi(jobs, machines, pop_size, mro=None):
    ...
    # Urutkan mesin dari yang paling free (mro terkecil) ke yang paling busy
    ...
    for _ in range(pop_size):
        ...
        for rank, job_idx in enumerate(indices):
            # 70% chance pilih dari mesin yang paling free, 30% random
            # Ini dorong populasi awal ke mesin free tapi tetap ada diversity
            if random.random() < 0.7:
                # Pilih dari sepertiga mesin paling free
...
def run_ccea(jobs, machines, config=None, machine_ready_offset=None):
    _validate_jobs(jobs)
    _validate_machines(machines)
    cfg = _validate_config(config or {})
    mro = machine_ready_offset or {}
    ...
    if len(jobs) == 1:
        ...
        machine_id = min(machines, key=lambda m: mro.get(m, 0.0))
        start_time = mro.get(machine_id, 0.0)
        end_time   = start_time + pt
        return {
            "makespan":    round(end_time, 2),
            "schedule":    [{
                "job_id":         job["job_id"],
                "machine_id":     machine_id,
                "start_time":     round(start_time, 2),
    ...
    logger.info("[CCEA] Start | Jobs: %d | Machines: %d | Subpop: %d×%d | MRO: %s",
                len(jobs), len(machines), n_subpop, subpop_size, mro)

    cache = FitnessCache()
    # Pass mro ke inisialisasi populasi agar bias ke mesin yang free
    subpopulasi = [_inisialisasi_populasi(jobs, machines, subpop_size, mro) for _ in range(n_subpop)]

    semua_individu = [ind for subpop in subpopulasi for ind in subpop]
    best_chrom     = max(semua_individu, key=lambda c: _compute_fitness(c, jobs, machines, cache, mro))
    best_fit       = _compute_fitness(best_chrom, jobs, machines, cache, mro)
    no_improve     = 0
    gen_final      = 0
    ...
    for gen in range(n_iter):
        ...
        subpopulasi, delta, new_best_chrom, new_best_fit = _sads_step(
            ...
            crossover_rate, mutation_rate, cache, best_chrom, best_fit, mro,
        )
        ...
        if gen_final % LOG_INTERVAL == 0:
            makespan, _ = hitung_makespan(best_chrom, jobs, machines, mro)
            logger.debug("[CCEA] Gen %d | Makespan: %.2f | No improve: %d", gen_final, makespan, no_improve)

        if no_improve >= early_stop_n:
            logger.info("[CCEA] Early stop gen %d", gen_final)
            break

    makespan, schedule = hitung_makespan(best_chrom, jobs, machines, mro)
